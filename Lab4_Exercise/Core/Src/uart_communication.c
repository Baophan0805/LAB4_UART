...
void uart_communication_fsm(ADC_HandleTypeDef hadc1, UART_HandleTypeDef huart2) {
	...
	case WAIT_RST:
		...
		break;
	case SEND_ADC:
		// Display ADC_value to console, status = WAIT_OK
		HAL_UART_Transmit(&huart2, (void*)str, sprintf(str, "!ADC=%lu#\r\n", ADC_value), 1000);
		status_uart = WAIT_OK;
		break;
	case WAIT_OK:
		// If command complete and command="OK", status=WAIT_RST and clearTimer
		if (command_flag == 1) {
			command_flag = 0;
		...
		// Else if after 3s system dont receive "OK", status = SEND_ADC
		if (timer_flag[1] == 1) {
			status_uart = SEND_ADC;
			setTimer(1, 3000);
		}
